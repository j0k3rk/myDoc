...
. 特殊关键字（virtual/override/new/base）的用法
+
[source, c++, numbered]
----
class A {
    public virtual void Func() // 虚函数，允许覆盖
    {
...
class D : A {
    public new void Func()  // 重写，隐藏基类的方法
    {
        Console.WriteLine("Func In D");
    }
    public new void Non_virtual()   // 重写，隐藏基类的方法
    {
...
----
// 将[port]替换为前一步定义的端口号
// 将[appPath]替换为包含 *.sln 文件的物理项目路径，原始路径需进行URL编码后使用
